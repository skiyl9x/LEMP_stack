...
sed -i "s/username_here/user/" ${site_dir}/wp-config.php $? "can't replase word"
sed -i "s/password_here/${db_pass}/" ${site_dir}/wp-config.php $? "can't replase word"
sed -i "s/localhost/db/" ${site_dir}/wp-config.php $? "can't replase word"
